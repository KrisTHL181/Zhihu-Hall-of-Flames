...
author: 知乎用户Q8d8bz
created: '2026-04-15'
source: http://zhuanlan.zhihu.com/p/2027709195227504895
---
...
**定义2.1** （Kolmogorov复杂度） 对于字符串$x \in \{0,1\}^*$，其Kolmogorov复杂度$K(x)$定义为：
...
**引理2.4** 若P=NP，则对于所有多项式时间可验证的语言$L \in \text{NP}$，有：
...
**引理2.5** 熵间隙$\Delta H = 0$蕴含对于所有$x$，存在多项式$p$使得：
...
**引理4.1** （压缩引理） 若$\Delta H(x) = 0$，则$x$可被压缩至长度$O(\log|x|)$的描述。

*证明* ：$\Delta H(x) = 0$意味着$K(x) = H_{\text{comp}}(x)$。由于$H_{\text{comp}}(x)$由多项式时间算法产生，该算法的索引长度为$O(\log|x|)$。$\square$

**引理4.2** （搜索引理） 若$K(x) \leq p(|x|)$，则可在$2^{p(|x|)} \cdot |x|^{O(1)}$时间内找到$x$的最短程序。
...
**问题5.1** （熵坍塌的定量刻画） 若P=NP，熵坍塌发生的速率是多少？具体地，是否存在常数$c$使得：
...
**问题5.2** （相变点的精确位置） 对于随机k-SAT，相变点$\alpha_c(k)$是否满足：
...
**问题5.3** （层级坍塌的精细结构） 若$\Sigma_i^p = \Sigma_{i+1}^p$，是否蕴含相应层级的熵坍塌？即：

$$
\forall L \in \Sigma_{i+1}^p, \exists p_i : K(x) \leq p_i(|x|)
$$

**问题5.4** （量子计算的熵间隙） 在BQP=P（量子多项式时间等于经典多项式时间）假设下，量子描述复杂度$K_Q(x)$与经典$K(x)$的关系如何？

**问题5.5** （证明复杂度的信息论下界） 能否建立证明长度$|P|$与熵间隙$\Delta H$的直接关系：
